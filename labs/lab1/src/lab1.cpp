...
int main()
{
    ...
    { 
        ...
        Rect rect;
        std::cout << "left_coord " << rect.get_left() << std::endl; // -695752112
        std::cout << "right_coord " << rect.get_right() << std::endl; // 29484
        std::cout << "top_coord " << rect.get_top() << std::endl; // -785553656
        std::cout << "bottom_coord " << rect.get_bottom() << std::endl; // 32764

        //Компилятор неявно определил конструктор по умолчанию и деструктор (тк не были объявлены другие конструктор и диструктор явно)

        //конструктор по умолчанию: нет параметров / все параметры имеют значения по умолчанию, не должно быть const
        //автоматический конструктор по умолчанию: выделяет память, не иницилизирует поля классы(мусор)
        //деструктор: ничего не делает, тк ничего не нужно очищать

        /*В общем случае: 
        конструктор по умолчанию: вызывает конструкторы для всех встроенных объектов класса; вызывает конструкторы базового класса
        деструктор по умолчанию: вызывает деструкторы встроенных объектов данного класса; вызывает деструкторы базовых классов.
        */
    } //вызван деструктор
    ...
    {
        /* 
        error: no matching function for call to ‘Rect::Rect(int, int, int, int)’
        если явно создан конструктор с параметрами, то автоматический конструктор по умолчанию не создается
        при создании Rect rect без параметров компилятор не нашел явный конструктор без параметров, поэтому ошибка
        */
...

    {
        ...
        Rect rect1(1, 2, 3, 4); //вызовется конструктор с параметрами, определили мы 
        Rect rect2 = rect1; // неявная иницилизация, вызовется конструктор копироаания
        Rect rect3(rect1); // прямая иницилизация копированием, вызовется конструктор копирования

        // компилятор неявно определил конструктор копирования, тк не было своего явного конструктора копирования
        // опредеился констуктор копирования, тк нет const, ссылок и он не находится в секции private

        /* в общем случае конструктор копирования:
        вызывает конструктор копирования базового класса 
        вызывает конструкторы копирования для встроенных объектов
        */
    } //вызваны деструкторы (сначала rect3, потом rect2, последний rect1)
     ...
     * 
     * 
     * Деструктор — это метод класса, который автоматически вызывается при уничтожении объекта
     * (выходе из области видимости или вызове delete). имя класса со знаком ~ в начале.
     * Порядок вызова деструкторов всегда обратный порядку создания
     * 
     * в моем классе Rect есть неявный автоматический деструктор(тк не объявила другие). 
     * ничего не делает, тк ничего не нужно очищать
     * 
     * В общем случае: 
     * деструктор по умолчанию вызывает деструкторы встроенных объектов данного класса; вызывает деструкторы базовых классов.
     */
...

    {
        ...
        Rect r1; //вызван конструктор по умолчанию для r1
        Rect *pR = new Rect(1,2,1,2); //вызван конструктор с параметрами 
        {
            Rect r2(r1); //вызван конструктор копирования для r2
            Rect arRect[2]; //вызваны два конструктора по умолчанию для arRect[0] и arRect[1]
            for(int i = 0; i < 3; i++)
            {
                static Rect r3 (i,i,i,i) ; //вызван конструктор c параметрами (1 раз, тк static)
                Rect r4(*pR); //вызван конструктор копирования для r4 (на каждой итерации)
                Rect r5(i,i,i,i); //вызван конструктор с параметрами для r5 (на каждой итерации)
            } //деструкторы для r5, r4 (на каждой итерации) 
        } //вызваны деструкторы для arRect[1] и arRect[0], r2
        delete pR; //деструктор для *pR
    } //вызван деструктор для r1
    // деструктор для r3 вызывается после завершения main(), тк static

    //кол-во конструкторов: r1+pR+r2+arRect[0]+arRect[1]+r3+3*(r4+r5) = 6 + 3*2 = 12
    //кол-во деструкторов: 3(r5+r4)+arRect[1]+arRect[0]+r2+pR+r1+r3 = 3*2 + 6 = 12
...

    {
        /*
        Rect r1;
        std::cout << r1.left_coord << std::endl;
        */

        //error: ‘int Rect::left_coord’ is private within this context
        //тк задали private, то нельзя обращаться и менять вне класса
    }
     ...
     * 2.имеет указатель this
     * 3.имеет доступ к private внутри класса
     * 4.может вызываться через объект класса
     */ 
     ...
     */
    {
        std::cout<<"\n"<<"Задание 1.9"<<std::endl;
        Rect r;
        r.inflate(5);
        std::cout << "left_coord " << r.get_left() << std::endl; 
        std::cout << "right_coord " << r.get_right() << std::endl; 
        std::cout << "top_coord " << r.get_top() << std::endl; 
        std::cout << "bottom_coord " << r.get_bottom() << std::endl; 
    ...
    {
        std::cout<<"\n"<<"Задание 1.10"<<std::endl;
        Rect r1(1,5,7,4), r2(2,7,9,4); // 2 вызова конструктора с параметрами для r1 и r2
        Rect r3 = bounding_rect(r1, r2);
        // 2 вызова конструктора копирования при передаче r1 r2 как параметров функции
        // вызов конструктора с параметрами при создании результата
        print_rect(r3);
    }
    ...
    {
        std::cout<<"\n"<<"Задание 1.11"<<std::endl;
        Rect rr(1, 5, 10, 2);
        std::cout<<rr.get_width()<<std::endl;
        std::cout<<rr.get_height()<<std::endl;
        std::cout<<rr.get_square()<<std::endl;
        rr.set_width(10);
        std::cout<<rr.get_width()<<std::endl;
        rr.set_height(30);
        std::cout<<rr.get_height()<<std::endl;
...

    {
        ...
        Bochka alch(100.0, 96.0); // бочка со спиртом
        Bochka water(100.0, 0.0);

        int i = 0;
        double conc = alch.concentr();
        while(conc > 50.0) {
            water.liter_of_another(alch);
            alch.liter_of_another(water);
            i++;
            conc = alch.concentr();
            std::cout << "После итерации " << i << ": " << conc << "%" << std::endl;
        }
        std::cout << "Концентрация стала <50% на итерации " << i << std::endl;
    }
    ...
    // Текущий вектор
    Matrix TekFib(2, 1, 0.0);
    TekFib.set(0, 0, 1.0);  // F[1]
    TekFib.set(1, 0, 0.0);  // F[0]
    ...
    {
        // Вывод текущего F[n]
        double tek_f = TekFib.get(0, 0);
        std::cout << "F[" << n << "] = " << tek_f << std::endl;
        
        // Вычисление следующего состояния
        Matrix next = PostFib.multiply(TekFib);
        
        // Обновление TekFib 
        TekFib.set(0, 0, next.get(0, 0));  // новый F[n+1]
        TekFib.set(1, 0, next.get(1, 0));  // новый F[n]
    }
...

    {
        ...
        MyString s2("hiiiii");
        s2.print();
        std::cout<<s2.get(3)<<std::endl;
        s2.set(3,'S');
        s2.print();
        s2.set_new_string("byeeee");
        s2.print();
        s2.read_line();
        s2.print();
    }
     ...
     * 1. добавлена переменная OBJECTS, которая потом использовалась при сборке:
     *      OBJECTS = build/rect.o build/barrel.o build/matrix.o build/mystring.o build/textwrapper.o
     *      build/debug.out: $(OBJECTS) src/lab1.cpp
     *           g++ -g -o build/debug.out src/lab1.cpp $(OBJECTS)
     * 2. было создано правило clean для очистки:
     *      .PHONY: clean
            clean:
     ...
     * Что поменялось:
     * 1. приватные поля
     * 2. методы, возвращающие координаты границ
     * 3. конструктор по умолчанию
     * 4. конструктор копирования
     * 5. метод set_all
     * 6. метод inflate
     * 7. метод move
     * 8. методы возврата/установки ширины и высоты
     * 
     */
