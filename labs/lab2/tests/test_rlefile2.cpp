...
// Тест для RleFile2 (композиция) со случайными данными
bool test_rlefile2_random() {
    ...
    // Запись
    {
        RleFile2 f(new BaseFile("data/test_rlefile2_random.rle", "w"));
    ...
    size_t total_read = 0;
    {
        RleFile2 reader(new BaseFile("data/test_rlefile2_random.rle", "r"));
    ...
    std::cout << (ok ? "Тест RleFile2 пройден!" : "Тест RleFile2 НЕ пройден!") << std::endl;
    return ok;
}

int main() {
    return test_rlefile2_random() ? 0 : 1;
}