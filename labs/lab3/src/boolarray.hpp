...
class BoolArray {
private:
    unsigned char* data;  // массив байтов (Каждый байт хранит 8 бит - логических значений)
...
public:
    ...
    BitReference operator[](int index); // для записи и чтения
    bool operator[](int index) const; // для константных объектов
