...
1. В локальном репозитории была создана ветка `prog-lab1` из актуальной версии main:
    ...
    ```
2. В этой ветке были сделаны все TODO к лабораторной номер 1 по дисциплине "Структуры данных", после чего изменения были сохранены, закоммичены и отправлены в удаленный репозиторий:
    ```bash
    git add .
    git commit -m "Исправление TODO"
    git push origin prog-lab1
    ```
3. Было сделано итоговое слияние ветки `prog-lab1` с `main`
